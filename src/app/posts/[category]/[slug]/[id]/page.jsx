...
const ArticlePage = () => {
  ...
  const relatedposts = postsByCategory.slice(0, 3);
  const [tags] = useFetchTags();
  const { postsFetchedByTag, haserror, loading } = useFetchPostsByTag(
    tags != null ? tags.id : ""
  );
    ...
    const fetchPostById = async () => {
      try {
        const response = await axios.get(
          `https://backend.mbote.cd/wp-json/wp/v2/posts/${id}`
        );
        ...
        <div className="container mx-auto py-[30px] px-4 md:px-0">
          <div className=" w-full lg:w-[953px] ">
            ...
            <div>
              <h1
                ...
                className="font-extrabold text-[36px] lg:text-[52px] leading-[1.2] "
              ></h1>
            </div>
          </div>
          <div className=" block md:flex gap-[20px] ">
            <div className=" px-0 md:px-[10px] w-full md:w-[848px] ">
              <div>
                <time class=" text-xs">
                  <Moment format="DD/MM/YY">{article.date}</Moment>
                </time>
              </div>
              <div className="flex gap-[6px] mt-4">
                <div>
                  <Image
                    ...
                    src="https://secure.gravatar.com/avatar/e54ac24b7860f183d1322ac30dec35ea?s=30&d=mm&r=g"
                    width="30"
                    height="30"
                    alt="avatar"
                  />
                ...
                <div>
                  <span className="text-xs">
                    Par
                    <Link
                      dangerouslySetInnerHTML={{ __html: article.author_name }}
                      className="ms-[3px] font-bold"
                      href={`/author/${article.author_name}`}
                    ></Link>
                  </span>
                </div>
              </div>
              ...
              <div>
                  ...
                  {relatedposts != null ? (
                    relatedposts.map((relatedpost) => (
                      <Relatedposts
                        ...
                        content={relatedpost.content.rendered}
                        featured_image={relatedpost.featured_image}
                        date={relatedpost.date}
                      />
                    ))
                  ) : (
                    <p>Loading similary posts</p>
                  )}
                </div>
                ...
                {
                  <div className=" mb-4">
                    {postsFetchedByTag != null ? (
                      postsFetchedByTag.map((articleByTag) => (
                        <ArticlesByTag
                          key={articleByTag}
                          title={articleByTag.title.rendered}
                          content={articleByTag.content.rendered}
                          featured_image={articleByTag.featured_image}
                          date={articleByTag.date}
                        />
                      ))
                    ) : (
                      <p>loading articles by tag</p>
                    )}
                  </div>
                }
              </div>
            </div>
            <div className="sticky top-0 mt-[88px] w-full md:w-[343px] ">
              {recentPosts != null ? (
                recentPosts.map((recentPost) => (
                  <RecentPosts
                    key={recentPost.id}
                    title={recentPost.title.rendered}
                    featured_image={recentPost.featured_image}
